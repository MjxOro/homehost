...
function NavApprovalsItem({
  ...
  return (
    <Link to="/approvals" className={active ? NAV_ITEM_ACTIVE : NAV_ITEM_IDLE}>
 ...
 * an off-canvas drawer below lg; the closed drawer is visibility-hidden and
 * inert so it never intercepts focus.
 */
export function AppLayout() {
  ...
  useEffect(() => {
    ...
    if (sidebar) {
      sidebar.inert = !isDesktop && !navOpen;
    }
  }, [isDesktop, navOpen]);
  ...
  const asideClass = navOpen
    ? "visible fixed inset-y-0 left-0 z-50 flex w-[280px] translate-x-0 flex-col gap-2 border-r border-line bg-ink-1 px-3.5 pb-4 pt-5 shadow-pop transition-transform duration-[0.22s] ease-[ease] lg:sticky lg:top-0 lg:z-auto lg:h-dvh lg:w-[264px] lg:shrink-0 lg:translate-x-0 lg:shadow-none"
    : "invisible fixed inset-y-0 left-0 z-50 flex w-[280px] -translate-x-[108%] flex-col gap-2 border-r border-line bg-ink-1 px-3.5 pb-4 pt-5 shadow-pop transition-transform duration-[0.22s] ease-[ease] lg:visible lg:sticky lg:top-0 lg:z-auto lg:h-dvh lg:w-[264px] lg:shrink-0 lg:translate-x-0 lg:shadow-none";

  const scrimClass = navOpen
    ? "visible fixed inset-0 z-[45] cursor-pointer bg-[rgba(4,8,10,0.6)] opacity-100 transition-opacity duration-200 ease-[ease] lg:hidden"
    : "invisible pointer-events-none fixed inset-0 z-[45] cursor-pointer bg-[rgba(4,8,10,0.6)] opacity-0 transition-opacity duration-200 ease-[ease] lg:hidden";
  ...
  return (
    <div className="flex min-h-dvh">
      ...
      <aside
        ...
        onClick={() => setNavOpen(false)}
        onBlur={(event) => {
          if (navOpen && !event.currentTarget.contains(event.relatedTarget))
            setNavOpen(false);
        }}
      >
        <div className="flex items-center gap-2.5 border-b border-line px-1.5 pb-3.5 pt-0.5">
          <ServerIcon className="size-[26px] text-accent" />
        ...
        {isPending ? (
          ...
          <SidebarNav user={session?.user ?? null} pathname={pathname} />
        )}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-line bg-ink-0/[0.88] px-[clamp(16px,4vw,44px)] py-2 backdrop-blur">
          <button
            ...
            type="button"
            className="inline-flex size-11 shrink-0 cursor-pointer items-center justify-center rounded-control border-0 bg-transparent text-text-2 transition-colors hover:bg-ink-2 hover:text-text-1 lg:hidden"
        ...
        <div className="flex items-start gap-2.5 border-b border-accent-line bg-accent-dim px-[clamp(16px,4vw,44px)] py-2.5 text-[13.5px] text-text-2">
          ...
          <p className="m-0 max-w-[110ch] leading-[1.5]">
            ...
            are involved. Approving a request only reserves capacity; nothing is
            ever provisioned.
          </p>
        </div>
        <main
